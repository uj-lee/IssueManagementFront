...
export function MyPage() {
  ...
  useEffect(() => {
    console.log("쿠키에 저장된 memberId:", cookies.jwt);
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const currentUser = devUsers.find(
            (user: User) => user.id === Number(cookies.memberId)
          );
          if (currentUser) {
        ...
        if (response.ok) {
          ...
          for (const project of projects) {
            const issueResponse = await fetch(
              `https://swe.mldljyh.tech/api/projects/${project.id}/issues`,
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (issueResponse.ok) {
              ...
              const userAssignedIssues = projectIssues.filter(
                (issue) =>
                  issue.assignee &&
                  issue.assignee.id === Number(cookies.memberId)
              );
              const userAssignedIssuesFiltered = userAssignedIssues.filter(
                (issue) => issue.status === "ASSIGNED"
              );
              const userResolvedIssuesFiltered = userAssignedIssues.filter(
                (issue) => issue.status !== "ASSIGNED"
              );
